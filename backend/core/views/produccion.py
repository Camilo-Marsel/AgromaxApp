...
class MataCaidaViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ["finca", "lote", "color_cinta", "semana_ã±o"]
...
class EmbarqueViewSet(viewsets.ModelViewSet):
    ...
    def tendencia(self, request):
        ...
        embarques.reverse()
        data = [{"id": e.id, "semana_año": e.semana_año, "fecha_embarque": str(e.fecha_embarque), "color_cinta": e.color_cinta, "total_cintas": e.total_cintas, "cajas_netas": e.cajas_netas, "ratio": float(e.ratio) if e.ratio is not None else None} for e in embarques]
    ...
    def validacion(self, request, pk=None):
        ...
        return Response({"embarque_id": embarque.id, "finca": finca_id, "lote": lote_id, "color_cinta": color, "semana_año": embarque.semana_año, "scope": "lote" if lote_id else "finca", "encintadas": int(encintadas), "caidas_reportadas": int(caidas), "esperadas": esperadas, "matas_corte": matas_corte, "matas_sin_reportar": matas_sin_reportar, "cajas_netas": embarque.cajas_netas, "cajas_esperadas": float(cajas_esperadas) if cajas_esperadas is not None else None, "ratio_actual": float(ratio_actual) if ratio_actual is not None else None, "desviacion_pct": float(desviacion) if desviacion is not None else None, "alerta_desviacion": alerta_desviacion, "alerta_matas_sin_reportar": alerta_matas, "umbral_desviacion_pct": float(umbral_desviacion), "umbral_matas": umbral_matas})
